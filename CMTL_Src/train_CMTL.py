...
from CMTL_Src.data_loader_CMTL import ImageDataLoader
from CMTL_Src.model_CMTL import *
...
def evaluate(sess, dataloader, input_image, gt_density, density):
    mae = 0
    mse = 0
    loss = 0
    num_samples = 0
...
def train(epoch_count, dropout, num_classes, batch_size):
    ...
    loss, cross_entropy, loss_den, mae, mse = model_loss(gt_label, class_logits, gt_density, density,
    ...
    with tf.Session() as sess:
        ...
        for epoch_i in range(1, epoch_count+1):
            ...
            for blob in data_loader:
                ...
                _, density_map, t_loss, t_cross_entropy, t_loss_den, t_mae, t_mse, num_global_step = \
                    sess.run([opt, density, loss, cross_entropy, loss_den, mae, mse, global_step],
                             feed_dict={input_image: im_data, gt_density: gt_den_data, gt_label: gt_label_data,
                                        ce_weights: ce_weights_data})
                ...
                print('Train -- epoch: {}, \tbatch: {}, \tglobal_step: {}\tloss: {}, loss_classfy: {}, '
                      '\tloss_density: {}, \tmse: {}, \tmae: {}'.format(epoch_i, batch_steps, num_global_step, t_loss,
                                                                        t_cross_entropy, t_loss_den, t_mse, t_mae))
                # break

            v_loss, v_mse, v_mae = evaluate(sess,data_loader_val, input_image, gt_density, density)
            print('-'*100)
